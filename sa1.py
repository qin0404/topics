...
def preprocess_course_data(courses_df):
    ...
    def parse_week_pattern(pattern):
        """
        预处理课程数据，解析教学周模式，支持复杂的不连续周模式
        改进：更好地处理空格、混合的单周和范围周模式，并增加更多错误处理
        """
        if not isinstance(pattern, str):
            # 默认使用所有教学周(1-12)，避免完全跳过课程
            print(f"警告: 教学周模式不是字符串类型 '{pattern}'，使用默认教学周(1-12)")
            return list(range(1, 13))

        pattern = pattern.strip()  # 移除前后空格
        if not pattern:  # 如果是空字符串
            print(f"警告: 教学周模式为空，使用默认教学周(1-12)")
            return list(range(1, 13))
        ...
        try:
            ...
            for part in [p.strip() for p in pattern.split(',')]:
                ...
                if '-' in part:
                    ...
                    try:
                        ...
                        if start > end:
                            print(f"警告: 教学周范围无效 '{part}'（起始大于结束），已调整")
                            start, end = end, start
                        weeks.extend(range(start, end + 1))
                    except ValueError:
                        print(f"警告: 无法解析教学周范围 '{part}'，跳过此项")
                        continue
                else:
                    # 处理单个周数（如"10"）
                    try:
            ...
            print(f"警告: 解析教学周模式 '{pattern}' 时出错: {str(e)}，使用默认教学周")
            return list(range(1, 13))
...
def preprocess_course_with_activities(courses_df):
    ...
    if original_count > filtered_count:
        print(f"已过滤掉 {original_count - filtered_count} 个 Real Size 为 0 的课程")
    ...
    for _, row in other_df.iterrows():
        ...
        # 新增：为指定了教室的课程设置更高的优先级
        if pd.notna(row.get('ROOM NAME')) and row['ROOM NAME'] != '':
            course_data['Priority'] = 1.5  # 介于lecture(1)和其他(2)之间的优先级
            course_data['Has_Designated_Room'] = True  # 标记有指定教室
        else:
            course_data['Priority'] = 2  # 默认中等优先级
            course_data['Has_Designated_Room'] = False
    ...
    for group_info in workshop_groups:
        ...
        if len(workshops) == 1:
            ...
            if pd.notna(workshop_entry.get('ROOM NAME')) and workshop_entry['ROOM NAME'] != '':
                ...
                workshop_entry['Priority'] = 1.5  # 提高优先级
            else:
        ...
        for group_idx, group in enumerate(merged_workshops):
            if len(group) == 1:
                ...
                if has_designated_room:
                    ...
                    main_workshop['ROOM NAME'] = designated_room  # 保留第一个发现的指定教室
                    main_workshop['Priority'] = 1.5  # 提高优先级
                else:
    ...
    print(f"其中，有指定教室的课程数: {designated_room_count}")
    print(f"合并的workshop组数: {sum(1 for _, row in result_df.iterrows() if row.get('Is_Merged', False))}")
    ...
    for course_idx, day_slots in course_day_time.items():
        for day, slots in day_slots.items():
            if len(slots) > 1:  # 同一门课在同一个星期几有多个不同的时间
                energy += len(slots) * hard_constraint_weight * 0.5  # 提高对同周不同时间段的惩罚
...
def generate_initial_solution_with_activities(regular_course_indices, index_to_course_row,
    ...
    # 按优先级排序课程索引（确保lecture和有指定教室的非lecture课程先被处理）
    lecture_indices = []
    designated_room_indices = []  # 新增：有指定教室的非lecture课程
    other_indices = []  # 其他普通课程
    ...
    for course_idx in lecture_indices:
        ...
        if course_row.get('Has_Designated_Room', False) and pd.notna(course_row.get('ROOM NAME')):
            ...
            if room_name in room_to_index:
                ...
                print(f"Lecture课程 {course_code} 使用指定教室: {room_name}")
            else:
                print(f"警告: Lecture课程 {course_code} 的指定教室 {room_name} 不存在")
        ...
        for week in teaching_weeks:
            ...
            if not success:
                ...
                successful_week_count += 1
                print(
                    f"  强制安排lecture课程 {course_code} 在第{week}周，星期{day}，时间槽{time_slot}，使用教室索引{chosen_room}")
    ...
    # 处理有指定教室的非lecture课程
    print(f"处理 {len(designated_room_indices)} 个有指定教室的非lecture课程...")
    ...
    for course_idx in designated_room_indices:
        ...
        if pd.notna(course_row.get('ROOM NAME')):
            ...
            if room_name in room_to_index:
                designated_room_idx = room_to_index[room_name]
                print(f"课程 {course_code} 使用指定教室: {room_name}")
            else:
                # 尝试查找标准化后匹配的教室
                ...
                for room, idx in room_to_index.items():
                    if normalize_room_name(room) == normalized_name:
                        ...
                        print(f"课程 {course_code} 使用标准化匹配的教室: {room}")
                        break

                if designated_room_idx is None:
                    print(f"警告: 课程 {course_code} 的指定教室 {room_name} 不存在，尝试使用最接近的教室")
                    # 找到容量最接近的教室
                    sorted_rooms = sorted(rooms_df.to_dict('records'),
                                          key=lambda x: abs(x['CAP'] - course_size))
                    if sorted_rooms:
                        designated_room_idx = room_to_index[sorted_rooms[0]['ROOM NAME']]
                        print(f"  选择容量为 {sorted_rooms[0]['CAP']} 的最接近教室")
                    else:
                        print(f"  错误: 找不到任何可用教室")
                        continue
        else:
            print(f"错误: 课程 {course_code} 标记为有指定教室，但未找到教室名称")
            continue
        ...
        # 为这门课程选择一个固定的时间槽，90%概率允许跨午休
        valid_fixed_slots = []
        for potential_start in range(1, 19 - duration_slots + 1):
            # 检查是否跨越午休时间
            if potential_start <= 6 and potential_start + duration_slots > 6:
                if random.random() < 0.95:  # 90%概率允许跨午休
                    valid_fixed_slots.append(potential_start)
                continue
            if potential_start <= 9 and potential_start + duration_slots > 9:
                if random.random() < 0.95:  # 90%概率允许跨午休
        ...
        for week in teaching_weeks:
            ...
            for attempt in range(max_attempts):
                ...
                if attempt > max_attempts // 2:
                    ...
                    for potential_start in range(1, 19 - duration_slots + 1):
                        # 检查是否跨越午休时间
                        if potential_start <= 6 and potential_start + duration_slots > 6:
                            if random.random() < 0.95:  # 95%概率允许跨午休
                                valid_start_slots.append(potential_start)
                            continue
                        if potential_start <= 9 and potential_start + duration_slots > 9:
                            if random.random() < 0.95:  # 95%概率允许跨午休
            ...
            # 如果尝试多次仍找不到合适的时间，则强制安排
            if not success:
                print(f"警告: 无法为指定教室课程 {course_code} 在第{week}周找到合适的时间，强制安排")
                ...
                success = True
                successful_week_count += 1
                print(f"  强制安排指定教室课程 {course_code} 在第{week}周，星期{day}，时间槽{time_slot}")
        ...
        if week_success:
            ...
            print(f"严重错误: 指定教室课程 {course_code} 完全无法安排")

        print(f"成功安排 {designated_room_success}/{len(designated_room_indices)} 门有指定教室的课程")

        # 现在处理其他普通课程
        print(f"处理 {len(other_indices)} 个普通课程...")
        ...
        for course_idx in other_indices:
            ...
            if not suitable_rooms:
                ...
                print(
                    f"警告: 课程 {course_code} ({activity_type}, 人数: {course_size}) 没有足够大的教室，尝试最接近的教室")
                ...
                if sorted_rooms:
                    ...
                    suitable_rooms = [closest_room_idx]
                    print(
                        f"  选择容量为 {sorted_rooms[0]['CAP']} 的最接近教室给课程 {course_code} (人数: {course_size})")
                else:
            ...
            for week in teaching_weeks:
                ...
                if not success:
                    # 新增：强制安排普通课程
                    print(f"警告: 无法为课程 {course_code} 在第{week}周找到合适的时间和教室，强制安排")

                    # 选择一个随机日期和时间
                    day = random.randint(1, 5)
                    time_slot = random.randint(1, 15)  # 较早的时间段
                    weekday_idx = 5 * (week - 1) + day
                    ...
                    if largest_rooms:
                        ...
                        success = True
                        successful_week_count += 1
                        print(f"  强制安排课程 {course_code} 在第{week}周，星期{day}，时间槽{time_slot}")
                    else:
                        print(f"  严重错误: 找不到任何教室给课程 {course_code}")
...
def generate_neighbor_with_activities(solution, regular_course_indices, index_to_course_row,
    ...
    修改：过滤掉lecture类型课程和有指定教室的课程，确保它们不会被修改教室。
    """
    ...
    # 过滤掉lecture课程和有指定教室的课程，确保它们不会被修改
    non_fixed_keys = []
    for key in solution.keys():
        course_idx = key[0]
        if course_idx in regular_course_indices:
            course_row = index_to_course_row.get(course_idx)
            if course_row and not course_row.get('Is_Lecture', False) and not course_row.get('Has_Designated_Room',
                                                                                             False):
                non_fixed_keys.append(key)

    # 如果没有非固定的课程，直接返回原解
    if not non_fixed_keys:
        return new_solution  # 没有可修改的非固定课程

    # 筛选出常规课程的键，确保这些键在当前解中存在
    regular_keys = [key for key in non_fixed_keys if key[0] in regular_course_indices]
    ...
    # 检查课程是否有指定教室
    has_designated_room = course_row.get('Has_Designated_Room', False)
    if has_designated_room:
        operation = 1  # 对于有指定教室的课程，只能改变时间，不能改变教室
    else:
...
def simulated_annealing_scheduling(
    ...
    try:
        ...
        if not zero_size_courses.empty:
            print(f"数据中有 {len(zero_size_courses)} 门Real Size为0的课程将被自动忽略")
        ...
        while iteration < max_iterations and temperature_worker > 0.1 and no_improvement < 10000:
            ...
            if energy_delta < 0 or random.random() < acceptance_probability:
                ...
                if current_energy_worker < best_energy_worker:
                    ...
                    best_energy_worker = current_energy_worker
                    no_improvement = 0
                    print("Worker {}: 迭代 {}, 温度 {:.2f}, 找到更好解: {}".format(worker_id, iteration, temperature_worker, best_energy_worker))
                else:
                    no_improvement += 1
            else:
                no_improvement += 1
            ...
            if iteration % 1000 == 0:
                ...
                print("Worker {}: 迭代 {}, 温度 {:.2f}, 当前能量 {}, 最佳能量 {}, 用时 {:.2f}秒".format(worker_id, iteration, temperature_worker, current_energy_worker, best_energy_worker, elapsed_time))
        print("Worker {}: 模拟退火完成，总迭代次数: {}, 最终温度: {:.2f}".format(worker_id, iteration, temperature_worker))
        print("Worker {}: 最佳解能量值: {}".format(worker_id, best_energy_worker))
        return best_solution_worker, best_energy_worker

    # 使用多线程并行运行多个模拟退火进程
    num_workers = 16  # 可根据实际情况调整线程数
    results = []
    ...
    final_temperature = 0.1  # 设置默认值
    final_iteration = max_iterations  # 设置默认值



    # --- 生成结果 ---
    print("\n模拟退火完成，总迭代次数: {}, 最终温度: {:.2f}".format(final_iteration, final_temperature))
...
if __name__ == "__main__":
    ...
    result_schedule, course_students, conflict_details = simulated_annealing_scheduling(
        ...
        cooling_rate=0.998,  # 更慢的冷却速率
        utilization_weight=1,  # 增加教室利用率权重
